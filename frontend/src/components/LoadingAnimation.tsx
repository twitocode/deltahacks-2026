...
export default function LoadingAnimation({ isLoading }: LoadingAnimationProps) {
  ...
  return (
    <div
      className={`absolute inset-0 z-50 flex items-center justify-center bg-black/30 transition-opacity duration-500 ${
    ...
    >
      <div className="flex flex-col items-center gap-8">
        ...
        <div className="grid grid-cols-3 gap-3">
          {Array.from({ length: 9 }).map((_, i) => {
            ...
            return (
              <div
                key={i}
                className={`w-12 h-12 rounded-lg border-2 border-white transition-all duration-300 ${
                  isActive
        ...
        <div className="flex flex-col items-center gap-2">
          <div
            key={message}
            className="text-white font-jetbrains text-sm tracking-[0.2em] font-medium uppercase animate-[fadeIn_0.5s_ease-out]"
          >
            {message}
          </div>
          <div className="text-white/80 font-jetbrains text-xs text-center max-w-[200px]">
            It&apos;s expected to take &gt; 1 min because of the cloud provider
          </div>
