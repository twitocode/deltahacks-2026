...
// Placeholder API functions (will be replaced with real backend calls)
async function fetchHeatmapData(_params: {
  latitude: number;
  longitude: number;
  age: number;
  sex: string;
  ...
  // Placeholder: Generate mock heatmap data around the given coordinates
  const { latitude, longitude, timeOffset } = _params;
  const points: Array<[number, number, number]> = [];
  const numPoints = 500;

  // Spread increases with time offset (simulating search area expansion)
  const spread = 0.05 + Math.abs(timeOffset) * 0.005;

  for (let i = 0; i < numPoints; i++) {
    // Generate random points in a rough circular pattern
    const angle = Math.random() * 2 * Math.PI;
    const distance = Math.random() * spread;
    const lat = latitude + Math.cos(angle) * distance;
    const lng = longitude + Math.sin(angle) * distance * 1.5;
    // Intensity decreases from center (higher near last known location)
    const intensity = Math.max(0.1, 1 - distance / spread);
...
function MapPage() {
  ...
  // Update heatmap when timeline changes (if we have data)
  useEffect(() => {
    if (lastKnownPosition && heatmapData.length > 0) {
      handleFindPerson();
    }
