...
def _latlon_to_index(lat: float, lon: float, terrain: TerrainModel) -> Tuple[int, int]:
...
def _is_valid_index(row: int, col: int, shape: Tuple[int, int]) -> bool:
...
def _calculate_direction_weights(
    ...
    row, col = _latlon_to_index(agent.lat, agent.lon, terrain)
    ...
    for dx, dy in directions:
        ...
        check_row, check_col = _latlon_to_index(check_lat, check_lon, terrain)
...
def step_single_agent_pure(
    ...
    if agent.steps_taken > 4:
        ...
        if random.random() < stop_prob:
            ...
            logs.append(
                {
                    "type": "stop",
                    "reason": f"ISRID User fatigue stop (prob={stop_prob:.1%})",
                }
            )
            return agent, logs
    ...
    lookahead_lon = agent.lon + dx * (
        lookahead_dist / (111320.0 * math.cos(math.radians(agent.lat)))
    )
    ...
    if distance_from_center_km > radius_km:
        ...
        logs.append(
            {
                "type": "stop",
                "reason": f"Exceeded search radius ({distance_from_center_km:.2f}km > {radius_km}km)",
            }
        )
        return agent, logs
    ...
    if new_elevation is None:
        agent.is_active = False
        logs.append(
            {"type": "stop", "reason": "Moved to invalid terrain (No elevation)"}
        )
        return agent, logs
...
class SARSimulator:
    ...
    async def run_simulation(
        ...
        if time_last_seen and current_time:
            ...
            # Guard against negative elapsed (if time_last_seen is in future, treat as just now)
            elapsed_minutes = max(0, elapsed_minutes)
        else:
            elapsed_minutes = 0

        # Total simulation: always run for 8 hours (480 minutes) to provide full timeline
        # We generate heatmaps at 15-minute intervals (33 data points: 0, 15, 30, ..., 480)
        total_minutes = 480
        num_steps = (
            total_minutes // self.settings.timestep_minutes
        ) + 1  # +1 to include endpoint
        ...
        with measure_time("simulation_loop"):
            for step in tqdm(range(num_steps), desc="Simulating", unit="step"):
                ...
                time_slices.append(
                    TimeSlice(
                        time_offset_minutes=time_offset, points=heatmap, grid=grid
                    )
                )
            ...
            for log in logs:
                if log["type"] == "decision":
                    tracker.log_decision(
                        updated_one.id, log["decision_type"], log["details"]
                    )
                ...
                with concurrent.futures.ProcessPoolExecutor(
                    ...
                    for future in concurrent.futures.as_completed(futures):
                        try:
                            updated_agent, _ = (
                                future.result()
                            )  # Ignore logs from parallel agents
    ...
    def _is_valid_index(self, row: int, col: int, shape: Tuple[int, int]) -> bool:
        """Check if grid indices are valid."""
