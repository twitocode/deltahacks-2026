...
class SARSimulator:
    ...
    def _step_single_agent(
        ...
        if agent.steps_taken > 4:
            ...
            if random.random() < stop_prob:
                ...
                tracker.log_stop(
                    agent.id, f"ISRID User fatigue stop (prob={stop_prob:.1%})"
                )
                return
        ...
        if agent.strategy == Strategy.DIRECTION_TRAVELING:
            ...
            tracker.log_decision(
                agent.id,
                "MOVE",
                f"Direction Traveling (Goal: {math.degrees(agent.heading):.0f}°, Actual: {math.degrees(actual_heading):.0f}°)",
            )
        else:
            ...
            tracker.log_decision(
                agent.id,
                "MOVE",
                f"Weighted Choice (Idx: {direction_idx}, Base: {cardinal})",
            )
        ...
        # Apply factors
        final_speed = tobler_speed_mps * (
            profile_speed / 1.317
        )  # Normalize profile to male base to scale Tobler
        ...
        tracker.log_energy(
            agent.id, old_energy, agent.energy, f"Walk cost + Slope {slope:.2f}"
        )
    ...
    def _calculate_direction_weights(
        ...
        for dx, dy in self.DIRECTIONS:
            ...
            check_row, check_col = self._latlon_to_index(check_lat, check_lon, terrain)
